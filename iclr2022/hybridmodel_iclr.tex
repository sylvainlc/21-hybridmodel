...
During the M4 competition, the hybrid model of \cite{} is based on a multiplicative exponential smoothing model as the time-series-specific model. However, on sporadic time series, this choice could lead to poor result and instability. In our approach, we decided to go deeper and provide a general framework able to deal with any kind of per-time-series models. In Section~\ref{sec:exp}, we present two versions of our framework. The first one use an  exponential smoothing as reference  similar to \cite{} and the second one use a TBATS model \cite{} which provides better results. The main complexity is in the next stage: training a global RNN to correct the per-time-series statistical models when the statistical models provide poor performance.
...
For the first parametric per-time-series models, we used existing Python or R librarys to estimate the different parameters $\statparam^n$. There are then fixed during the training of the recurrent models. The proposed hybrid model introduced in this numerical section is the TBATS model of \cite{XXX}. This model is also compared to an hybrid model with an additive exponential smoothing model as a reference close to \cite{XXX}. For $\classifier$ and $\predictor$, we built an architecture summarized in Figure~\ref{fig:rnnarchi}. There are both composed of 3 LSTM layer and a different final Dense layer to provide the correct output. After the $\classifier$ final dense layer, we add a sigmoid activation to return a probability. We use 2 Adam otpimizers with the same learning rate equal to ??? and the batch size is  64. The eval set is used to fix the $\threshold$ parameter that determine, depending of the output of $\predictor$, when  tuse the $\predictor$ correction. 
...
As benchmarks, we chose several well-known statistical methods and deep learning approaches. Using the R package \texttt{forecast} and the Python packages \texttt{statsmodels, tbats} , we compute for each time series a prediction with the following methods: \textit{snaive}, \textit{ets}, \textit{stlm}, \textit{thetam}, \textit{tbats} and \textit{auto.arima}. The \textit{snaive} forecasts are only the repetition of the last same past period. The \textit{ets} model is an additive exponential smoothing with a level component and a seasonal component. The \textit{stlm} approach uses a multiplicative decomposition and models the seasonality adjusted time series with an exponenantial smoothing model. The \textit{Thetam} model decomposes the original signal in $\theta$-lines, forecasts each one separately and recomposes them to produce the final forecast and  \textit{tbats} is a powerful statistical model using a trigonometrical seasonality modelization. Finally,  \textit{auto.arima} is the R implementation of ARIMA model with an automatic selection of the best parameters. A complete description and references for these models can be found in \citep{packageforecast}. As a deep learning approach, we considered a full LSTM (\textit{lstm}) neural network composed of 3 LSTM layers of shape 50 and a final Dense layer of shape 52.
...
\textcolor{blue}{hybrid model correct all time series and not only a a group of hundred. WIP}  \\
In a first time, we present the accuracy of all our methods without using our weak signals.for our 3 metrics and for each model, we compute the average and standard deviation on all our sequences on the final year. Results are shown in the Table ???.  
\begin{itemize}
...
\begin{table*}
  \caption{Results summary on our Fashion dataset. For each metric, we compute the average and the standard deviation on all the test set.}
  \label{sample-table}
  \centering
  \begin{tabular}{lllllll}
    ...
    \hline
     \textit{snaive} & 0.871 & 0.255 & 25.29 & 14.90 & 1. & 0.\\
     \textit{ets} & 0.801 & 0.274 & 24.18 & 17.51 & 0.944 & 0.286\\
     \textit{thetam} & 0.849 & 0.364 & 24.58 & 17.07 & 0.980 & 0.333\\
     \textit{stlm} & 0.765 & 0.265 & 22.22 & 13.77 & 0.885 & 0.188\\
     \textit{tbats} & 0.734 & 0.254 & 21.19 & 13.79 & 0.847 & 0.171\\
     \textit{auto.arima} & 0. & 0. & . & . & . & 0.\\
     \textit{lstm} & 0.765 & 0.278 & 21.00 & 11.42 & 0.872 & 0.160\\
     \textit{rshm-ets} & 0. & 0. & . & . & . & 0.\\
     \textit{\textbf{rshm-tbats}} & 0.726 & 0.242 & 21.09 & 13.60 & 0.840 & 0.157\\
     \textit{oracle} & 0.709 & 0.235 & 20.55 & 13.30 & 0.812 & 0.153\\
    \hline
  \end{tabular}
\end{table*}


We also compute for each method a confusion matrix in order to evaluate in what matter, each candidate is able to detect and accurately forecast increasing/decreasing trends.
...
\begin{table*}
  ...
  \begin{tabular}{llll}
    ...
    &  dec-trend-pred  & flat-trend-pred & inc-trend-pred  \\
    \hline
	true-dec-trend & 33 & 166 & 9 \\
    true-flat-trend & 13 & 596 & 12 \\
    true-inc-trend & 7 & 139 & 25 \\
    \hline
  \end{tabular}
\end{table*}
...
In a seconde time, we evaluate how our framework is able to understand and leverage our weak signals. The Table ??? presents the result of our three candidates that we train with our weak signals : \textit{lstm-ws}, \textit{rshm-ets-ws} and \textit{rshm-tbats-ws}.  
...
\begin{table*}
  ...
  \begin{tabular}{lllllll}
    ...
    \hline
     ...
     \textit{\textbf{rshm-tbats-ws}} & 0. & 0. & . & . & 0. & 0.\\
    \hline
  \end{tabular}
\end{table*}

Again, we show the result of our model with external signal on the classifier task. The Table ???/???/??? present their results.
