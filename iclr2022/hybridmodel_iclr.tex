...
& = \sum_{U_T^n} p(U_T^n|\ts^n_{1:T+h},\rnnparam^{(t)}) \times \log p_\theta(\ts^n_{T+1:T+h},U_T^n|\ts^n_{1:T}) \\
& = \sum_{U_T^n} p(U_T^n|\ts^n_{1:T+h},\rnnparam^{(t)}) \times \log[p_\theta(U_T^n|\ts^n_{1:T})p_\theta(\ts^n_{T+1:T+h}|U_T^n,\ts^n_{1:T})]
\end{align}
