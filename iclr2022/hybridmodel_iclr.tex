...
$$
where $\{\varepsilon^n_{T+i}\}_{i\leqslant 1}$ are independent \textcolor{blue}{centered} random variables, $\{\remainder^n_{T+i}\}_{i\leqslant 1}$ are non null remainders and $U^n_T$ an unobserved regime switching Bernoulli random variable.
...
To train our hybrid model, we split our dataset in 3 parts, train,eval and test set. We use the 3 first years as our train set, we keep the 4th year for our eval set and we hide the last year for our test set. In order to increase our train set, we use a commom moving window as in Figure~\ref{fig:architecture}.
