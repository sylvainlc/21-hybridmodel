...
%$$
At this stage, we assume that we have two different regimes. In the regime 0, the first parametric forecast succed at catching all the information of the $n$-th sequence. Conversely, in the regime 1, the first parametric forecast is unaccurate and it remains a non-forecasted quantity that we note $\remainer$. Our regime-switching system is then :

$$
\ts^n_{T+i} = \tspred^{stat,n}_{T+i|T} + \hiddenregime^n_T * \remainer^{n}_{T+i} + \varepsilon^n_{T+i}\,,
$$
where $\{\varepsilon^n_{T+i}\}_{i\leqslant 1}$ are independent \textcolor{blue}{centered} random variables and $U^n_T$ an unobserved regime switching Bernoulli random variable.
%In the regime 1, the first parametric forecast is unaccurate and it remains a non-forecasted quantity that we note $\remainer$.
%For this regime, we have for all $i \in \{1,\ldots,\lag\}$ :
%$$
%\ts^n_{T+i} = \tspred^{stat,n}_{T+i|T} + \remainer^{n}_{T+i} + \varepsilon^n_{T+i}\,,
%$$
...
$$
\ts^n_{T+i}  = \tspred^{stat,n}_{T-w+i|T-w} +  \mathbf{1}_{\classifier(\rnnwindow^n_T) > \threshold} \times \predictor(\rnnwindow^n_T)_{T+i} \times \meants^n_T + \varepsilon^n_{T+i}\
$$
