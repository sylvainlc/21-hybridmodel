...
\begin{align*}
\ts^n_{T+i}  &= \tspred^{stat,n}_{T-w+i|T-w} +  \hat{\hiddenregime}^n_T \cdot \hat{\remainder}^{n}_{T+i} + \varepsilon^n_{T+i}\,, \\
...
\begin{equation}
\label{eq:full:model}
\ts^n_{T+i}  = \tspred^{stat,n}_{T-w+i|T-w} +  \mathbf{1}_{\classifier(\fullconcatinput^n_T) > \threshold} \cdot \predictor(\fullconcatinput^n_T)_{T+i} \cdot \meants^n_T + \varepsilon^n_{T+i}\,,
\end{equation}
