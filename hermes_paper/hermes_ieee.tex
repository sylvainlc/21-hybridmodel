...
Looking at the final result of the M4 competition, some models reach a higher accuracy than the HERMES framework on the weekly dataset. All of them are ensemble framework : a combination of different models in order to increase the robustness of the final forecast and the global accuracy. If this paper, only individual models were compared on the fashion dataset and on the M4 dataset. However, the HERMES framework, as any individual method, is perfectly suited to be include in an ensembling model like the FFORMA approach proposed in \cite{MONTEROMANSO202086}.
