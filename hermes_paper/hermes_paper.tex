...
We provide the first dataset dedicated to fashion trends forecasting. Our fashion dataset is built with the suprising alliance of a high throughput computer vision system and experts of the fashion sphere. A team of deep learning engineers designed and trained a collection of vision neural networks at detecting clothes details: color, cloth type (pants, shoes, top etc...), form, size, texture etc... Then a second team of fashion experts aggregates these previous attributes and created thousands of fashion trends.
...
Our raw sequences have all 261 time steps, from 2015-01-05 to 2019-12-31 with weekly values and no missing values. Each value represents the number per week of analysed images where our vision system detected a specific fashion trend. However, in this paper, we present a normalized version of our signals in order to remove strong social media biases. Due to the increasing use of social media and user's change of behaviours, each trend is normalized by its top hierarchical parents and consequently lie between 0 and 1. We deseasonized the top hierarchical parent before normalized to not remove the season of the original sequence. An example of normalization can be found in Figure~\ref{fig:normalization} for a  trend.
At the end, the proposed fashion dataset contains $\numberts$ anonymized fashion trends for men and women, on 9 different categories and 5 geozones. An overview of it can be found in Table~\ref{tab:fashiondataset}.
...
Firstly, we created a specific fashion-oriented panel of micro-influencers. With the same methodology than our main signals, we can represent our fashion trends life on social media on this specific population. we named \textit{fashion-forwards} this first weak signal. For instance, an impressive emerging fashion shoes trend with its linked \textit{fashion-forwards} weak singal is represented in Figure~\ref{fig:oneemergingtrend}. 
...
As we want to detect shifts between our main signal and our fashion forward signal and also normalize our weak signal, we compute the following input for our hybrid model :
...
At the end, we created 3 weak signals $\ws^{l}$,$\ws^{l}$ and $\ws^{h}$. They are frequently sparse for micro trends and they often lack interest in common fashion trends. However, in several examples, they are fundamental to detect the future fashion evolution.
...
For the first parametric per-time-series models, we used existing Python or R libraries to estimate the different parameters $\statparam^n$. %During the M4 competition, parameters of the exponential smoothing of the porposed hybrid model could move during the training. In our framework,
%$\{\statparam^n\}_{1\leqslant n \leqslant N}$ are fixed during the training of the recurrent models. 
In this section we propose two versions of HERMES depending of the choice of local parametric models. The first one use as predictors an additive exponential smoothing model as a reference close to \cite{smyl2020hybrid}. The second one use as predictors the TBATS model of \cite{doi:10.1198/jasa.2011.tm09771} and  achieves striking accuracy results on our fashion dataset. For the neural network part, we built an architecture summarized in Figure~\ref{fig:rnn_architecture}. It is composed of 3 LSTM layer of shape 50 and a final Dense layer to provide the correct output dimension \textcolor{red}{Peut-on discuter/justifier ce choix ?}. We use a classical Adam optimizer with a learning rate set at 0.001 or 0.005, a batch size equal to 64 and the following loss function:
...
\begin{subfigure}{1.\textwidth}
  \centering
  \includegraphics[width=1.\linewidth]{figure/cn_female_texture_horizontalstipe}
...
With these investigations, we can make two important remarks. First, our hybrid framework \textit{hermes-tbats} achieves the first position in term of global accuracy on both datasets. Due to the strengh of its per-time-series predictor TBATS, our hybrid model succeds at correcting some of its weaknesses and logically reach a striking final accuracy. Secondly, we can note that the accuracy of the full neural network \textit{lstm} decreases when the dataset size is smaller. With a small dataset of 100 time series, a local statistical model like \textit{tbats} or \textit{stlm} can largely outperform its accuracy level. Learn how to compute a full prediction from scratch is a complex task and high-dimensional recurrent neural networks require  large amount of data. Nevertheless, with our hybrid framework, the RNN task is largely simplified. Our achitectures logicaly needs less data to be trained and to perform well.
...
\begin{table*}
  \caption{Results summary on our 1000ts Fashion dataset and 100ts Fashion Dataset.For each metric, we compute the average on all our time series. For our two apporaches using neural network, we trained 10 models with different seeds and we display the mean and the standard deviation of our 10 results.}
%  \centering
 \resizebox{\textwidth}{!}{
...
%    \hline
     ...
     \textit{tbats} && 0.734 & - \\
     \textbf{\textit{hybrid-nows}} && \textbf{0.719} & 0.002 \\
  \end{tabular}
  \quad
  \begin{tabular}{l||llll}
...
%    \hline
	\hline
     ...
     \textit{stlm} && 0.742 & - \\
     \textit{tbats} && 0.745 & - \\
     \textbf{\textit{hybrid-nows}} && \textbf{0.739} & 0.003 \\
  \end{tabular}
  }
\label{tab:1000metricresults}
\end{table*}
