...
\And
Sylvain Le Corff \\
Samovar, T\'el\'ecom SudParis, d\'epartement CITI, \\
...
\begin{table}
  ...
  \caption{M4 weekly dataset overview. For each category, we give the number of sequences and the average length.}
\label{tab:m4dataset}
  \centering
  \resizebox{1.\width}{!}{
  \begin{tabular}{l||llllll}
    %hline
    \\
    &  \textbf{Demographic}  & \textbf{Finance} & \textbf{Industry} & \textbf{Macro} & \textbf{Micro} & \textbf{Other} \\
    \hline
    \hline
    \\
	\textbf{Number of sequences} & 24 & 164 & 6 & 41 & 112 & 12 \\
	\\
    \textbf{average length} & 1659 & 1237 & 834 & 1264 & 473 & 1598 \\
    \textbf{minimal length} & 1615 & 260 & 356 & 522 & \textbf{93} & 470 \\
  \end{tabular}
  }
\end{table}
...
\end{subfigure}
\caption{Examples of time series from the M4 weekly dataset.}
...
\end{subfigure}
\caption{\textit{hermes-tbats} forecast examples on the M4 weekly dataset. In green the prediction of the per-time-series predictors \textit{tbats}. In red the final forecast of our HERMES hybrid model \textit{hermes-tbats}.}
\label{fig:m4pred}
\end{figure}

However, on the M4 weekly dataset final accuracy, some models reach a higher accuracy than the HERMES framework. Except one approach, all of them are a combination of different models. It is commom in time series forecasting to aggregated various approachs to increase the robusness of the final forecast and in the same time achieve a better global accuracy. The best model on the M4 weekly dataset is not an ensembling and with a OWA equal to 0.739, outperforms the \textit{hermer-tbats} and all the M4 proposed models on the M4 weekly sub-dataset. This framework called \textit{Forecast Pro} is a model selection approach depending where a collection of parametric model are trained and selected depending of time series properties. A description and interesting remarks are discussed is \citep{DARIN2020135} 
An interessant reseach perspective can be to replace the tbats per-time-series predictor by this individual models which seems more suited for the M4 weekly forecasting task.
