...
\begin{figure*}
  ...
  \caption{From social media to fashion time series. a) A complete image dataset of 150 millions of pictures is collected from social media users localized on 5 strategic markets. b) A visual recognition pipeline is applying on images. Global fashion items are detected with a collection of fine-grain attributes. c) Result are aggregated by fashion trend definition over the time and normalized in order to remove social media bias. The fashion time series are created.}
\label{fig:pipeline}
\end{figure*}
...
\section{The HERMES approach: a new hybrid model for time series forecasting}
\label{sec:hybrid}
