...
\begin{table}
  \caption{Results summary on 1000 time series and 100 time series of the fashion dataset. The MASE average on all the time series is computed. For the two approaches using a neural network, 10 models with different seeds are trained. the mean and the standard deviation of the 10 results are displayed. \textcolor{orange}{Models with a $^*$ in their name have access to the influencers external signal.}\vspace{0.5cm}}
 \centering
